...
class HttpRequest:
    ...
    def __str__(self) -> str:
        ...
        if self._body:
            body_preview = (
                self._body[:10] + "..." if len(self._body) > 10 else self._body
            )

        return (
            f"HttpRequest(url='{self._url}', method='{self._method}', "
    ...
    class Builder:
        ...
        def build(self) -> "HttpRequest":
            ...
            # Validation logic
            if self._method in ["POST", "PUT"] and (
                not self._body or not self._body.strip()
            ):
                print(
                    f"Warning: Building {self._method} request without a body for URL: {self._url}"
                )
